...
int main(int argc, char *argv[])
{
    ...
    static int arr[1000000]; // array to be sorted
    static int sorted[1000000];
    static int linearlySorted[1000000];
...
#pragma omp parallel for private(j)
    for (i = 0; i < arraySize; i++)
    {
        ...
        sorted[x] = arr[i];
        // printf("current Thread %d start: %d end: %d\n", omp_get_thread_num(), (omp_get_thread_num() * arraySize / omp_get_num_threads()), ((omp_get_thread_num() + 1) * arraySize / omp_get_num_threads()) - 1);
    }
    ...
    printf(">> Speed Up (sequential / parallel) : %f\n", ((stop.tv_sec - start.tv_sec) + (stop.tv_usec - start.tv_usec) / 1000000.0f) / (openmp_end - openmp_start));
    return 0;
}